...
const MyPage = () => {
  ...
  const loadUserInfo = async () => {
    ...
    const results = await getRequest( 'http://112.186.20.155:8299/api/v1/user/load', getToken(TOKEN_NAME))
  ...
  const onClickSave = useCallback(async()=> {
      ...
      const results = await postRequest('http://112.186.20.155:8299/api/v1/member/profile', data, getToken(TOKEN_NAME))
  ...
  const getTarget = useCallback(async()=> {
    ...
    const results = await getRequest('http://112.186.20.155:8299/api/v1/member/view?pk=' + encodeURIComponent(User.email) , getToken(TOKEN_NAME))
