...
const RegisterBarcode = () => {
    ...
    const getData = useCallback(async () => {
        ...
        const res = await getRequest('http://112.168.150.239:8299/api/v1/barcode/view?pk=' + getParameter('pk'), getToken(TOKEN_NAME))
    ...
    const onsubmitForm = useCallback(async (e) => {
        ...
        const res = await postRequest('http://112.168.150.239:8299/api/v1/barcode/register', data, getToken(TOKEN_NAME))
    ...
    const onsubmitFormUpdate = useCallback(async (e) => {
        ...
        const res = await postRequest('http://112.168.150.239:8299/api/v1/barcode/update', data, getToken(TOKEN_NAME))
            ...
            <InnerBodyContainer>
                ...
                <WhiteBoxContainer>
                    <form onSubmit={isUpdate ? onsubmitFormUpdate : onsubmitForm}>
                        ...
                        <NormalInput title={'바코드 이름'} value={name} onChangeEvent={setName}
                                     description={'이름 혹은 별칭을 입력하세요'}/>
                        <NormalInput title={'바코드 규칙'} value={code} onChangeEvent={setCode}
                                     description={'규칙이 되는 기본 코드를 입력하세요 ex) SIZL-0000-0000'}/>
                        <DropdownInput title={'바코드 타입'} target={indexList[type]} contents={indexList}
                                       onChangeEvent={(v) => setType(v)}/>

                        <br/>
                        <ListHeader title="선택 항목"/>
                        <NormalInput title={'바코드 설명'} value={spec} onChangeEvent={setSpec}
                                     description={'바코드의 설명을 입력하세요'}/>
