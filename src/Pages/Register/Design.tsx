...
const RegisterDesign = () => {
    ...
    const getData = useCallback(async () => {
        ...
        const res = await getRequest('http://255.255.255.255:8299/api/v1/mold/view?pk=' + getParameter('pk'), getToken(TOKEN_NAME))
    ...
    const onsubmitForm = useCallback(async (e) => {
        ...
        const res = await postRequest('http://255.255.255.255:8299/api/v1/mold/register' + pk, data, getToken(TOKEN_NAME))
    ...
    const onsubmitFormUpdate = useCallback(async (e) => {
        ...
        const res = await postRequest('http://255.255.255.255:8299/api/v1/mold/update', data, getToken(TOKEN_NAME))
            ...
            <InnerBodyContainer>
                ...
                <WhiteBoxContainer>
                    <form onSubmit={isUpdate ? onsubmitFormUpdate : onsubmitForm}>
                        ...
                        <DropdownInput title={'금형종류'} target={indexList[type]} contents={indexList}
                        ...
                        <NormalInput title={'제조번호'} value={no} onChangeEvent={setNo}
                                     description={'제조사가 발급한 제조사 번호를 입력하세요'}/>
                        <NormalFileInput title={'금형 상 사진'} name={paths[0]} thisId={'moldPhotoo0'}
                                         onChangeEvent={(e) => addFiles(e, 0)}
                                         description={isUpdate ? oldPaths[0] : '장치 측면에 붙어있는 명판(혹은 스티커)을 사진으로 찍어 등록해주세요'}/>
                        <NormalFileInput title={'금형 하 사진'} name={paths[1]} thisId={'moldPhoto1'}
