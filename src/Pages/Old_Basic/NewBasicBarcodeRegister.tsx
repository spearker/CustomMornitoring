...
import {API_URLS, registerBasicItem, loadBasicItem} from '../../Api/mes/basic'
...
const indexList = ['기계 기본정보', '주변장치 기본정보', '금형', '품목 기본정보', '원자재 기본정보']
...
const NewBasicBarcodeRegister = () => {
    ...
    }, [inputData, selectMachine, rules, barcodeImg, reason, type, typeFilter])
            ...
            <InnerBodyContainer>
                ...
                <BigWhiteBoxContainer>
                    <div>
                        ...
                        <ColorCustomPickerModal select={selectMachine} onClickEvent={(e) => setSelectMachine(e)} noOnClick={getParameter('pk') === "" ? false : true}
                        ...
                        {typeFilter === 0 && <>
                            ...
                            {getParameter('pk') === "" && 30 > rules.length && <WithTextBox title={''} padding={rules.length > 0 && rules[0] !== null ? '0 0 18px 0' : '18px 0'}>
                                <InputBox onClick={() => {
                                    let temp = _.cloneDeep(rules)
                                    temp.push('')
                                    setRules(temp)
                            ...
                            <InputBox>
                                <input type="text" disabled
                                    value={typeFilter === 0 ? rules.map(v => {
                                        ...
                                        if (v !== null) return v + `-`
                                    }).join().replace(/,/g, '') : ''} placeholder={typeFilter === 0 ? '바코드 번호 생성 버튼을 눌러주세요.' : '등록완료시 자동 생성됩니다.' }/>
                                {/* <button onClick={() => {
