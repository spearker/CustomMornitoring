...
const FullMonitoring = () => {
    ...
    const getList = useCallback(async () => {
        ...
        const res = await getRequest(`http://203.234.183.22:8299/api/v1/monitoring?type=press&from=mobile`, getToken(TOKEN_NAME))
        setIsFirstLoad(true)
        if (res === false) {
    ...
    return (
        <FullSizeContainer index={'monitoring'}>
                    ...
                    <div style={{position: 'absolute', display: 'inline-block', top: 0, right: 0}}>
                        {
                            openList.length === list.length ?
                                <MonitoringOptionButton title={'전체 접기'} color={'#b3b3b3'} onClickEvent={() => {
                                    setOpenList([])
                                }}/>
                                :
                                <MonitoringOptionButton title={'전체 펼치기'} color={'#b3b3b3'} onClickEvent={() => {
                                    setOpenList(
                                        list.map((v: IMonitoringList) => {
                                            return v.pk!
                                        })
                                    )
                                }}/>
                        }
                        <MonitoringOptionButton title={'전체화면 끄기'}
                                                onClickEvent={() => history.push('/monitoring/press')}/>
                        <MonitoringOptionButton title={'항목 골라보기'} color={'#b3b3b3'} onClickEvent={() => {
                            setIsPopup(true)
                            setCheckList(selectedList)
                        }}/>
                        <MonitoringOptionButton title={'초기화'} color={'#b3b3b3'} onClickEvent={() => {
                            onClickRefresh()
                        }}/>
                    </div>
...

                    {
                        list.map((v: IMonitoringList, i) => {
                            if (statusFilter === 'all') {
                                ...
                                return (
                                    <MonitoringCardSmall key={'m-' + i} contents={v}
                                                         optionList={selectedList.length === 0 ? v.info_list.map((m) => {
            ...
            <SearchModalContainer
                ...
                isVisible={isPopup} onClickClose={() => {
                setIsPopup(false)
            }} title={'항목 선택 하기'}>
                ...
                <div style={{width: '100%', maxHeight: 380, overflowY: 'scroll'}}>
                    {
                        optionList.map((v, i) => {
                            ...
                            return (
                                ...
                                <SearchedList key={'s' + i} pk={String(v)} widths={['100%']}
                                              contents={[transferCodeToName('title', v)]} isIconDimmed={false}
                                              isSelected={checkList.find((c) => c === v) ? true : false}
                                              onClickEvent={() => {
