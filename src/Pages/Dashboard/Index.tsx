...
const Dashboard = () => {
  ...
  const getStatus = useCallback(async (index: number)=>{
    const results = await getRequest('http://61.101.55.224:8299/api/v1/dashboard/machine/' + index, getToken(TOKEN_NAME))
  ...
  const getTasks = useCallback(async (index: number)=>{
    const results = await getRequest('http://61.101.55.224:8299/api/v1/task/list/' + filter, getToken(TOKEN_NAME))
  ...
  const onClickTaskStatus = useCallback(async(pk: string, value:string)=>{
    ...
    const results = await postRequest('http://61.101.55.224:8299/api/v1/task/status', data,getToken(TOKEN_NAME))
