...
const MachineMaintenance = () => {
  ...
  const getSearchList = useCallback(async (e) => {
    e.preventDefault();
    const results = await getRequest('http://112.168.150.239:8299/api/v1/preserve/list?keyword=' + keyword + '&orderBy=' + option + '&type=' + type, getToken(TOKEN_NAME))
    if (results === false) {
  ...
  const onClickDelete = useCallback(async (id) => {
    ...
    const results = await postRequest('http://112.168.150.239:8299/api/v1/preserve/delete', { pk: id }, getToken(TOKEN_NAME))
  ...
  return (
      <DashboardWrapContainer index={5}>
        ...
        <InnerBodyContainer>
          ...
          <InfoTable indexList={index} widthList={[ '70%', '20%' ]} type={'maintenance'} pkKey={'pk'}
                     onClickLinkUrl="/maintenance/update?type=machine&pk=" contents={list}
