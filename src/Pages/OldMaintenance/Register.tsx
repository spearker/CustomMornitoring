...
const RegisterMaintenance = () => {
  ...
  const getData = useCallback(async () => {
    ...
    const res = await getRequest('http://112.168.150.239:8299/api/v1/preserve/view?pk=' + getParameter('pk') + '&type=' + getParameter('type'), getToken(TOKEN_NAME))
  ...
  const onsubmitForm = useCallback(async (e) => {
    ...
    const res = await postRequest('http://112.168.150.239:8299/api/v1/preserve/register', data, getToken(TOKEN_NAME))
  ...
  const onsubmitFormUpdate = useCallback(async (e) => {
    ...
    const res = await postRequest('http://112.168.150.239:8299/api/v1/preserve/update', data, getToken(TOKEN_NAME))
  ...
  const onClickSearch = useCallback(async (e) => {
    ...
    const res = await getRequest(`http://112.168.150.239:8299/api/v1/common/search?keyword=${keyword}&type=${type}&orderBy=1`, getToken(TOKEN_NAME))
        ...
        <InnerBodyContainer>
          ...
          <WhiteBoxContainer>
            <form onSubmit={isUpdate ? onsubmitFormUpdate : onsubmitForm}>
                    ...
                    <>
                      <div style={{ display: 'flex', alignItems: 'center', marginTop: 12, marginBottom: 24 }}>
                        <div style={{ paddingLeft: 1, paddingTop: 5 }}>
                          <input type="radio" id="cb" name="type" checked={type === 'machine' ? true : false}
                                 onClick={(e) => {
                                   setType('machine')
                                 }}/>
                        ...
                        <div style={{ paddingLeft: 1, paddingTop: 5 }}>
                          <input type="radio" id="cb2" name="type" checked={type === 'peripheral' ? true : false}
                                 onClick={(e) => {
                                   setType('peripheral')
                                 }}/>
                        ...
                        <div style={{ paddingLeft: 1, paddingTop: 5 }}>
                          <input type="radio" id="cb3" name="type" checked={type === 'mold' ? true : false}
                                 onClick={(e) => {
                                   setType('mold')
                                 }}/>
                      ...
                      {/* 팝업 여는 버튼 + 보전 주변장치 추가 */}
                      {
                        type === 'peripheral' ?
                            ...
                            <AddInput title={'보전 주변장치'} icType="solo" onlyOne={list.length > 0 ? true : false}
                                      onChangeEvent={() => {
                                        setIsPoupup(true);
                                        setCheckList(list);
                                        setKeyword('')
                      ...
                      {
                        type === 'mold' ?
                            <AddInput title={'보전 금형'} icType="solo" onlyOne={list2.length > 0 ? true : false}
                                      onChangeEvent={() => {
                                        setIsPoupup2(true);
                                        setCheckList2(list);
                                        setKeyword('')
                      ...
                      {
                        type === 'machine' ?
                            <AddInput title={'보전 기계'} icType="solo" onlyOne={list3.length > 0 ? true : false}
                                      onChangeEvent={() => {
                                        setIsPoupup3(true);
                                        setCheckList3(list3);
                                        setKeyword('')
              ...
              <NormalNumberInput title={'권장 점검 주기(일)'} value={term} onChangeEvent={setTerm}
                                 description={'권장 점검주기를 입력해주세요 (일 단위)'}/>
