...
const CompanySetting = () => {
  ...
  const onClickSave = useCallback(async () => {
    ...
    const results = await postRequest('http://192.168.0.47:8299/api/v1/member/update', data, getToken(TOKEN_NAME))
  ...
  */
  const getTarget = useCallback(async () => {
    ...
    const results = await getRequest('http://192.168.0.47:8299/api/v1/member/view?pk=' + getParameter("pk"), getToken(TOKEN_NAME))
  ...
  const getList = useCallback(async () => {
    const results = await getRequest('http://192.168.0.47:8299/api/v1/member/teams/list?keyword=' , getToken(TOKEN_NAME))
    if (results === false) {
  ...
  const getDataSubTeams = useCallback(async () => {
    ...
    const results = await getRequest('http://192.168.0.47:8299/api/v1/member/teams/list?pk=' + targetTeam.pk + '&keyword=' , getToken(TOKEN_NAME))
    if (results === false) {
     ////alert('데이터를 불러 올 수 없습니다. 잠시후 이용하세요.')
    } else {
      if (results.status === 200) {
