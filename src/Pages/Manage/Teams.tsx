...
const TeamsSetting = () => {
  ...
  const onClickAdd = useCallback(async (id) => {
    ...
    if(id === 0){
      ...
      const results = await postRequest('http://61.101.55.224:8299/api/v1/member/teams/register' ,data, getToken(TOKEN_NAME))
      if (results === false) {
  ...
  const getList = useCallback(async () => {
    ...
    const results = await getRequest('http://61.101.55.224:8299/api/v1/member/teams/list?keyword=' + keyword, getToken(TOKEN_NAME))
    if (results === false) {
     ////alert('데이터를 불러 올 수 없습니다. 잠시후 이용하세요.')
    } else {
      if (results.status === 200) {
  ...
  const getDataSubTeams = useCallback(async () => {
    ...
    const results = await getRequest('http://61.101.55.224:8299/api/v1/member/teams/list?pk=' +  target!.pk  + '&keyword=' + keyword, getToken(TOKEN_NAME))
    if (results === false) {
     ////alert('데이터를 불러 올 수 없습니다. 잠시후 이용하세요.')
    } else {
      if (results.status === 200) {
  ...
  const getSearchList = useCallback(async (e) => {
    e.preventDefault();
    const results = await getRequest('http://61.101.55.224:8299/api/v1/teams/list?keyword=' + keyword, getToken(TOKEN_NAME))
    if (results === false) {
     ////alert('데이터를 불러 올 수 없습니다. 잠시후 이용하세요.')
    } else {
      if (results.status === 200) {
  ...
  const onClickDelete = useCallback(async (id, abl) => {
    ...
    const results = await postRequest('http://61.101.55.224:8299/api/v1/member/teams/delete', { pk: id }, getToken(TOKEN_NAME))
  ...
  const onClickModify = useCallback(async (id, value) => {
    ...
    const results = await postRequest('http://61.101.55.224:8299/api/v1/member/teams/update', { pk: id, name: value }, getToken(TOKEN_NAME))
