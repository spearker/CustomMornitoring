...
const BasicMoldRegister = () => {
  ...
  const getData = useCallback(async () => {
    ...
    const res = await getRequest('http://255.255.255.255:8299/api/v1/mold/load?pk=' + getParameter('pk'), getToken(TOKEN_NAME))
  ...
  const onsubmitForm = useCallback(async () => {
    ...
    const res = await postRequest('http://255.255.255.255:8299/api/v1/mold/register', data, getToken(TOKEN_NAME))
          ...
          <WhiteBoxContainer>
            {
              ...
              <form onSubmit={isUpdate ? onsubmitFormUpdate : onsubmitForm} target={'iframe'}>
                ...
                <InputContainer title={"투입 품목"}>
                  <ProductionPickerModal select={input_material} onClickEvent={setInput_material} text={'투입품목'}
                                         width={true} type={0} style={{ width: 'calc(100% - 100px)' }}
                                         innerWidth={'100%'}/>
                </InputContainer>
                <InputContainer title={"생산 품목"}>
                  <ProductionPickerModal select={output_material} onClickEvent={setOutput_material} text={'생산품목'}
                ...
                {/*  loadDataUrl={isUpdate? `http://255.255.255.255:8299/api/v1/mold/load?pk=${pk}` :''}*/}
