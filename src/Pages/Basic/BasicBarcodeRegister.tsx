...
const BasicBarcodeRegister = () => {
  ...
  const onsubmitFormUpdate = useCallback(async (e) => {
    ...
    const res = await postRequest('http://112.168.150.239:8299/api/v1/barcode/standard/update', data, getToken(TOKEN_NAME))
  ...
  const onsubmitForm = useCallback(async (e) => {
    ...
    const res = await postRequest('http://112.168.150.239:8299/api/v1/barcode/standard/register', data, getToken(TOKEN_NAME))
    if (res === false) {
      ////alert('////alert('[SERVER ERROR] 요청을 처리 할 수 없습니다.')')
          ...
          <WhiteBoxContainer>
            {
              ...
              <>
                ...
                <CustomPickerModal onClickEvent={(e) => console.log(e)} text={'세부 항목을 검색해주세요.'}
                                   type={indexType[inputData.type]}/>
                {
                  inputData.rules.length > 0 && inputData.rules[0] !== null &&
                  <BarcodeText><br/><span>현재 규칙</span><br/>{inputData.rules.map(v => {
                    if (v !== null) return v + `-`
                ...
                {/* /!*  loadDataUrl={isUpdate? `http://112.168.150.239:8299/api/v1/barcode/standard/load?pk=${pk}` :''}*!/*/}
