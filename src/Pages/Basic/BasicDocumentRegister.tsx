...
const BasicDocumentRegister = () => {
  ...
  const getItems = useCallback(async()=>{
    ...
    const res = await getRequest('http://192.168.0.14:8299/api/v1/item/involved?category=' + necessary['standard_type'].data.id, getToken(TOKEN_NAME))
    setIsChange(false)
    if(res === false){
      //TODO: 에러 처리
  ...
  const getData = useCallback(async()=>{
    ...
    const res = await getRequest('http://192.168.0.14:8299/api/v1/document/load?pk=' + getParameter('pk'), getToken(TOKEN_NAME))
  ...
  const onsubmitFormUpdate = useCallback(async(e)=>{
    ...
    const res = await postRequest('http://192.168.0.14:8299/api/v1/document/update', data, getToken(TOKEN_NAME))
  ...
  const onsubmitForm = useCallback(async(e)=>{
    ...
    const res = await postRequest('http://192.168.0.14:8299/api/v1/document/register', data, getToken(TOKEN_NAME))
