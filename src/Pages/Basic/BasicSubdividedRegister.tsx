...
const BasicSubdividedRegister = () => {
  ...
  const getData = useCallback(async () => {
    ...
    const res = await getRequest('http://255.255.255.255:8299/api/v1/subdivided/load?pk=' + getParameter('pk'), getToken(TOKEN_NAME))
  ...
  const onsubmitFormUpdate = useCallback(async (e) => {
    ...
    const res = await postRequest('http://255.255.255.255:8299/api/v1/subdivided/update/', data, getToken(TOKEN_NAME))
  ...
  const onsubmitForm = useCallback(async (e) => {
    ...
    const res = await postRequest('http://255.255.255.255:8299/api/v1/subdivided/register', data, getToken(TOKEN_NAME))
          ...
          <WhiteBoxContainer>
            {
              ...
              <form onSubmit={isUpdate ? onsubmitFormUpdate : onsubmitForm}>
                ...
                <NormalInput title={'세분화 이름'} value={inputData.name} description={''} onChangeEvent={(input) => {
                  let temp = _.cloneDeep(inputData);
                  temp.name = input;
                  setInputData(temp)
                }}/>
                ...
                <NormalInput title={'설명'} value={inputData.description} description={''} onChangeEvent={(input) => {
                  let temp = _.cloneDeep(inputData);
