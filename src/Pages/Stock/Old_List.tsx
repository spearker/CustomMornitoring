...
const StockList = () => {
  ...
  const getSearchList = useCallback(async (e) => {
    e.preventDefault();
    const results = await getRequest('http://192.168.0.46:8299/api/v1/material/list?keyword=' + keyword + '&orderBy=' + option, getToken(TOKEN_NAME))
    if (results === false) {
  ...
  const onClickDelete = useCallback(async (id) => {
    ...
    const results = await postRequest('http://192.168.0.46:8299/api/v1/material/delete', { pk: id }, getToken(TOKEN_NAME))
  ...
  return (
      <DashboardWrapContainer index={8}>
        ...
        <InnerBodyContainer>
          ...
          <InfoTable indexList={index} pkKey={'pk'} type={'material'} onClickEvent={onClickList} onClickEventName={'입고'}
