...
const MaterialStock = () => {
  ...
  const getSearchList = useCallback(async (e) => {
    e.preventDefault()
    const results = await getRequest('http://61.101.55.224:18299/api/v1/material/list?keyword=' + keyword + '&orderBy=' + option, getToken(TOKEN_NAME))
    if (results === false) {
  ...
  const onClickDelete = useCallback(async (id) => {
    ...
    const results = await postRequest('http://61.101.55.224:18299/api/v1/material/delete', {pk: id}, getToken(TOKEN_NAME))
    const tg = id
