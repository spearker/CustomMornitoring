...
const DesignList = () => {
  ...
  const getSearchList = useCallback(async (e)=>{
    e.preventDefault();
    const results = await getRequest('http://112.168.150.239:8299/api/v1/mold/list?page='+page+'&keyword='+ keyword +'&type=' + option ,getToken(TOKEN_NAME))
  ...
  const onClickDelete = useCallback(async (id)=>{
    ...
    const results = await postRequest('http://112.168.150.239:8299/api/v1/mold/delete', {pk:id}, getToken(TOKEN_NAME))
    const tg = id
    //console.log('--select id : ' + id)
    if(results === false){
  ...
  return (
      <DashboardWrapContainer index={0}>
        ...
        <div style={{position:'relative'}}>
          ...
          <InfoTable indexList={index} type={'mold'} pkKey={'pk'} typeKey={'mold_label'} typeChanger={machineCodeToName} onClickLinkUrl="/update/design?pk=" contents={list} onClickRemove={onClickDelete}/>
