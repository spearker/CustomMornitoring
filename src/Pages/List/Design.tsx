...
const DesignList = () => {
  ...
  const getSearchList = useCallback(async (e)=>{
    e.preventDefault();
    const results = await getRequest('http://293.234.183.22:8299/api/v1/mold/list?page='+page+'&keyword='+ keyword +'&type=' + option ,getToken(TOKEN_NAME))
  ...
  const onClickDelete = useCallback(async (id)=>{
    ...
    const results = await postRequest('http://293.234.183.22:8299/api/v1/mold/delete', {pk:id}, getToken(TOKEN_NAME))
    const tg = id
    //console.log('--select id : ' + id)
    if(results === false){
