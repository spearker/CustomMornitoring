...
const MachineList = () => {
  ...
  const getSearchList = useCallback(async (e)=>{
    e.preventDefault();
    const results = await getRequest('http://61.101.55.224:8299/api/v1/machine/list?page='+page+'&keyword='+ keyword +'&type=' + option, getToken(TOKEN_NAME))
  ...
  const onClickDelete = useCallback(async (id)=>{
    ...
    const results = await postRequest('http://61.101.55.224:8299/api/v1/machine/delete', {pk:id}, getToken(TOKEN_NAME))
    const tg = id;
    console.log('--select id : ' + id)
    if(results === false){
