...
const TaskRegister = () => {
  ...
  const getRecommendData = useCallback(async () => {
    ...
    const res = await getRequest('http://183.99.194.242:8299/api/v1/task/recommend?pk=' + encodeURIComponent(keyword), getToken(TOKEN_NAME))
  ...
  const onClickSearch = useCallback(async (e) => {
    ...
    const res = await getRequest(`http://183.99.194.242:8299/api/v1/${type}/search?keyword=` + keyword, getToken(TOKEN_NAME))
  ...
  const onClickSearchMaterial = useCallback(async (e) => {
    ...
    const res = await getRequest('http://183.99.194.242:8299/api/v1/material/search?keyword=' + keyword, getToken(TOKEN_NAME))
  ...
  const onsubmitFormUpdate = useCallback(async (e) => {
    ...
    const res = await postRequest('http://183.99.194.242:8299/api/v1/task/update', data, getToken(TOKEN_NAME))
  ...
  const onsubmitForm = useCallback(async (e) => {
    ...
    const res = await postRequest('http://183.99.194.242:8299/api/v1/task/register', data, getToken(TOKEN_NAME))
  ...
  const getData = useCallback(async () => {
    ...
    const res = await getRequest('http://183.99.194.242:8299/api/v1/task/view?pk=' + getParameter('pk'), getToken(TOKEN_NAME))
  ...
  const onSubmitFile = useCallback(async (id) => {
    ...
    const res = await postRequest('http://183.99.194.242:8299/api/v1/task/file', data, getToken(TOKEN_NAME))
      ...
      <DashboardWrapContainer index={7}>
        <InnerBodyContainer>
          ...
          <WhiteBoxContainer>
            <form onSubmit={isUpdate ? onsubmitFormUpdate : onsubmitForm}>
              <div style={{ borderBottom: 'solid 0.5px #d3d3d3' }}>
                ...
                <PlaneInput value={description} description={'상세 업무내용 작성 (200자 미만)'} onChangeEvent={setDescription}
                            fontSize={'14px'}/>
              </div>
              <div style={{
                display: 'flex',
                ...
                <div style={{ width: '55%' }}>
                  ...
                  <AddInput line={false} title={'생산제품 (*필수)'} icType="solo" onlyOne={list.length > 0 ? true : false}
                            onChangeEvent={() => {
                              setIsPoupup(true);
                              setCheckList(list);
                              setKeyword('')
                ...
                <div style={{ width: '45%', textAlign: 'left' }}>
                  ...
                  <InputBox type="number" value={amount} onChange={(e: React.ChangeEvent<HTMLInputElement>): void => {
              ...
              <div style={{ marginTop: 16, marginBottom: 24 }}>
...

                {
                  option === 0 && !isUpdate ?
                      list.length < 1 ?
                          <p style={{ padding: 60, textAlign: 'center', color: "#aaaaaa" }}>생산할 제품(자재) 먼저 선택 후, 추천 공정을
                            선택 할 수 있습니다</p>
                          :
                          recommend === undefined || recommend.length == 0 ?
                              <p style={{ padding: 60, textAlign: 'center', color: "#252525" }}>해당 제품(자재)을 생산하기 위한 추천공정이
                              ...
                              <div>
                                {
                                  recommend.map((v, i) => {
                                    return (
                                        <ProcessTable pk={String(i)} select={selectIndex} contents={v}
                      ...
                      list2.length < 1 ?
                          ...
                          :
                          <div>
                            <ProcessTable pk={''} contents={list2} indexList={[ '명칭', '', '기계 정보', '금형정보', '자재정보' ]}
          ...
          <SearchModalContainer
            ...
            <>
              <SearchInput description={'작업자를 검색해주세요'} value={keyword} onChangeEvent={(e) => setKeyword(e.target.value)}
                           onClickEvent={onClickSearch}/>
              <div style={{ width: '100%', marginTop: 20 }}>
                {
                  isSearched ?
          ...
          <SearchModalContainer
            ...
            <>
              <SearchInput description={'공유자를 검색해주세요'} value={keyword} onChangeEvent={(e) => setKeyword(e.target.value)}
                           onClickEvent={onClickSearch}/>
              <div style={{ width: '100%', marginTop: 20 }}>
                {
                  isSearched ?
