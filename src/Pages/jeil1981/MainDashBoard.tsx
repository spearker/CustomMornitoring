...
const dummy = [
    {pk: '213', type: 'press', name: '프레스 1호기', status: 'active', photo: jeilpress1, is_connect: true,},
    {pk: '211r1323', type: 'press', name: '프레스 2호기', status: 'active', photo: jeilpress2, is_connect: true,},
    {pk: 'ewa', type: 'press', name: '프레스 3호기', status: 'active', photo: jeilpress3, is_connect: true,},
    {pk: '211wev23', type: 'press', name: '프레스 4호기', status: 'active', photo: jeilpress4, is_connect: true,},
    {pk: 'qweeq', type: 'press', name: '프레스 5호기', status: 'active', photo: jeilpress5, is_connect: true,},
]
...
const MainDashboard = () => {
    ...
    const getStatus = useCallback(async (index: number) => {
        const results = await getRequest('http://61.101.55.224:18299/api/v1/dashboard/machine/' + index, getToken(TOKEN_NAME))
    ...
    const getTasks = useCallback(async (index: number) => {
        const results = await getRequest('http://61.101.55.224:18299/api/v1/task/list/' + filter, getToken(TOKEN_NAME))
    ...
    const onClickTaskStatus = useCallback(async (pk: string, value: string) => {
        ...
        const results = await postRequest('http://61.101.55.224:18299/api/v1/task/status', data, getToken(TOKEN_NAME))
                    ...
                    <div style={{
                        ...
                        <div style={{width: '4%'}}>
                            <img onClick={page > 1 ? () => onClickChangePage(page - 1) : () => {
                                 ...
                                 style={{cursor: 'pointer', width: 24, height: 24, margin: 10, zIndex: 3}}/>
                        </div>
                        <div style={{
                            flexWrap: 'wrap',
                            flexDirection: 'row',
                            ...
                            <div style={{marginTop: 7,}}>
                                <DotPagenation stock={Math.ceil(dummy.length / 6)} selected={page}
                                               onClickEvent={onClickChangePage}/>
                            </div>
                        </div>

                        <div style={{width: '4%'}}>
                            <img
                                onClick={page < Math.ceil(dummy.length / 6) ? () => onClickChangePage(page + 1) : () => {
