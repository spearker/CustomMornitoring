...
const MainDashboard = () => {
    ...
    const getStatus = useCallback(async (index: number) => {
        const results = await getRequest(`${SF_ENDPOINT}/api/v1/dashboard/machine/` + index, getToken(TOKEN_NAME))
    ...
    const getTasks = useCallback(async (index: number) => {
        const results = await getRequest(`${SF_ENDPOINT}/api/v1/task/list/` + filter, getToken(TOKEN_NAME))
    ...
    const onClickTaskStatus = useCallback(async (pk: string, value: string) => {
        ...
        const results = await postRequest(`${SF_ENDPOINT}/api/v1/task/status`, data, getToken(TOKEN_NAME))
