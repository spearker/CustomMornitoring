...
const MainDashboard = () => {
    ...
    const getStatus = useCallback(async (index: number) => {
        const results = await getRequest('http://222.100.89.245:8299/api/v1/dashboard/machine/' + index, getToken(TOKEN_NAME))
    ...
    const getTasks = useCallback(async (index: number) => {
        const results = await getRequest('http://222.100.89.245:8299/api/v1/task/list/' + filter, getToken(TOKEN_NAME))
    ...
    const onClickTaskStatus = useCallback(async (pk: string, value: string) => {
        ...
        const results = await postRequest('http://222.100.89.245:8299/api/v1/task/status', data, getToken(TOKEN_NAME))
