...
const Old_OutsourcingCompanyRegister = () => {
  ...
  const getData = useCallback(async () => {
    ...
    const res = await getRequest('http://183.99.194.242:8299/api/v1/outsourcing/load?pk=' + getParameter('pk'), getToken(TOKEN_NAME))
  ...
  const onsubmitFormUpdate = useCallback(async (e) => {
    ...
    const res = await postRequest('http://183.99.194.242:8299/api/v1/outsourcing/update', data, getToken(TOKEN_NAME))
  ...
  const onsubmitForm = useCallback(async (e) => {
    ...
    const res = await postRequest('http://183.99.194.242:8299/api/v1/outsourcing/register', data, getToken(TOKEN_NAME))
              ...
              document.id !== '' || isUpdate == true ?
                  ...
                  <form onSubmit={isUpdate ? onsubmitFormUpdate : onsubmitForm}>
                    ...
                    <FileAddInput title={'사업자 등록증 사진'} target={`image`} description={`사진 형식의 파일을 업로드 해주세요.`}
                                  value={inputData.photo} onChangeEvent={(input) => setInputData(`photo`, input)}/>
