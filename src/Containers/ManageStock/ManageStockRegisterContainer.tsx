      ...
      <ContainerMain>
        ...
        <div>
          <table style={{color: 'black'}}>
            <tr>
              <td>• 품목(품목명)</td>
              <td><ProductionPickerModal select={selectItem} onClickEvent={(e) => setSelectItem(e)}
                                         text={'품목(품목명)을 선택해주세요.'}/></td>
            </tr>
            <tr>
              <td>• 자재 종류</td>
              <td><RegisterDropdown type={'string'} onClickEvent={(e: string) => setProductionType(e)}
                                    select={productionType} contents={productionTypeList} text={'자재 종류를 선택해 주세요'}/></td>
            </tr>
            <tr>
              <td>• 재고 분류</td>
              <td><RegisterDropdown type={'string'} onClickEvent={(e: string) => setStockType(e)} select={stockType}
                                    contents={stockTypeList} text={'재고 분류를 선택해 주세요'}/></td>
            </tr>
            <tr>
              <td>• 재고량</td>
              <td><Input type={'number'} placeholder="재고량을 입력해 주세요."
                         onChangeText={(e: number) => setContractData({...contractData, amount: e})}/></td>
            </tr>
            <tr>
              <td>• 보관장소</td>
              <td><RegisterDropdown type={'string'} onClickEvent={(e: string) => setSafetyStock(e)} select={safetyStock}
                                    contents={safetyStockList} text={'보관 장소를 선택해 주세요'}/></td>
            </tr>
            <tr>
              <td>• 안전재고</td>
              <td><Input type={'number'} placeholder="안전재고량을 입력해주세요."
                         onChangeText={(e: number) => setContractData({...contractData, amount: e})}/></td>
            </tr>
          </table>
        </div>
