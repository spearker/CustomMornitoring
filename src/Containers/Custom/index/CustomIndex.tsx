...
const NewDashboard = () => {
    ...
    const getList = useCallback(async () => { // useCallback
        ...
        if (res) {
            ...
            const getInfo = res.info_list.map((Info) => {
                const goal = Info.goal.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                const produced = Info.produced.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')

                return {...Info, goal: goal, produced: produced}
            })
