...
const CustomerRegister = ({match}: Props) => {
  ...
  const onsubmitFormUpdate = useCallback(async () => {
    ...
    const data = {
      ...
      manager_phone: phoneM ? String(phoneM) === '' ? null : String(phoneM) : null,
    ...
    const tempUrl = `${API_URLS['customer'].update}`
    const res = await postCustomerDelete(tempUrl, data)
  ...
  const ckBisNo = (bisNo: string) => {
    ...
    // return ((10 - ((sum + Math.floor(key[8] * Number(bisNo[8]) / 10)) % 10)) % 10) == Number(bisNo[9]);
    if (bisNo.length === 10) {
      return true
    }
    return false
  ...
  const onsubmitForm = useCallback(async () => {
    ...
    const tempUrl = `${API_URLS['customer'].create}`
    const res = await postCustomerDelete(tempUrl, data)
