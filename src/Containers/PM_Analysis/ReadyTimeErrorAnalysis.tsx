...
const ReadyTimeErrorAnalysisContainer = () => {
  ...
  const convertTimeFunc = (numberTime: number) => {
    let tmpNumberTime: number = numberTime
    let timeString: string = ''
    if (tmpNumberTime / (60 * 60) >= 1) {
      timeString = `${Math.floor(tmpNumberTime / (60 * 60))}시간 `
      tmpNumberTime = tmpNumberTime % (60 * 60)
    }

    if (tmpNumberTime / 60 >= 1) {
      timeString = `${timeString}${Math.floor(tmpNumberTime / 60)}분 `
      tmpNumberTime = tmpNumberTime % 60
    }

    timeString = `${timeString}${tmpNumberTime.toFixed(0)}초 `
    return timeString
  }
              ...
              <div>
                <div className={'itemDiv'} style={{float: 'left', display: 'inline-block'}}>
                  <p style={{
                    ...
                    fontSize: 20,
                    fontWeight: 'bold'
                  }}>{pressName} {error.error_name} 분석</p>
                </div>
                <div style={{marginRight: 30, paddingTop: 25,}}>
                  <CalendarDropdown unLimit select={selectDate}
                        ...
                        custom: function ({dataPointIndex}) {
                          return (
                            `<div style='width: 200px; height: 80px; background-color: white; display: flex; flex-direction: column; justify-content: center;'>
                                     ...
                                     <p>에러 해결 시간 평균 : ${convertTimeFunc(Number(dots[dataPointIndex].resolve))}</p>
                                  </div>`
                          )
                        }
                      }
