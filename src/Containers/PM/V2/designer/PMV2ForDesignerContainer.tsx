...
const PMV2ForDesignerContainer: React.FunctionComponent = () => {
  ...
  const goToPressDashboard = React.useCallback((type: string) => {
    history.push(`/pm/v2/dashboard/press/${type}`)
  }, [])
    ...
    <div>
      ...
      <Container>
        <div style={{ display: 'flex' }}>
          <Grid background={desp1000} top={63} left={235} onClick={() => goToPressDashboard('0')}/>
          <Grid background={desp800} top={63} left={401} onClick={() => goToPressDashboard('1')}/>
          <Grid background={desp600} top={63} left={558} onClick={() => goToPressDashboard('2')}/>
          <Grid background={desp400} top={63} left={717} onClick={() => goToPressDashboard('3')}/>
        </div>
        <Grid background={despJail400} top={353} left={1131} onClick={() => goToPressDashboard('4')}/>
      </Container>
    </div>
  )
}
