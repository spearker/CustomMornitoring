...
const WorkerContainer = ({match}: Props) => {
  ...
  }, [selectDate, match.params.pk, searchValue, page, option, saveKeyword])


  const getList = useCallback(async (isSearch?: boolean) => { // useCallback
    //TODO: 성공시
    Notiflix.Loading.Circle()
    const tempUrl = `${API_URLS['production'].history}?pk=${match.params.pk !== undefined ? match.params.pk : ''}&from=${selectDate.start}&to=${selectDate.end}&limit=15&page=${isSearch ? 1 : page.current}&keyword=${saveKeyword}`
    const res = await getProjectList(tempUrl)
    if (res) {
      const getWorker = res.info_list.map((v, i) => {
  ...
  }, [list, selectDate, page, match.params.pk, option, searchValue, saveKeyword])
