...
const DefectiveRegisterContainer = ({ match }: Props) => {
  ...
  const onsubmitFormUpdate = useCallback(async () => {
    ...
    const res = await postRequest('http://255.255.255.255:8299/api/v1/defective/update/', data, getToken(TOKEN_NAME))
  ...
  const onsubmitForm = useCallback(async () => {
    ...
    const res = await postRequest('http://255.255.255.255:8299/api/v1/defective/register', data, getToken(TOKEN_NAME))
  ...
  return (
      <div>
        ...
        <WhiteBoxContainer>
          ...
          <NormalNumberInput title={'불량 개수'} value={amount} onChangeEvent={setAmount} description={'불량 개수를 입력하세요.'}
                             width={120}/>
