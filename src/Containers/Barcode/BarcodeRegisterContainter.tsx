        ...
        <div>
            ...
            <WhiteBoxContainer>
                <div>
                    ...
                    <DropdownInput title={'바코드 종류'} target={indexBarcodeType[0]} contents={indexBarcodeType} onChangeEvent={(input)=>setInputData(`barcode_type`, BarcodeType[0])}/>
                    <DropdownInput title={'항목'} target={indexList[type]} contents={indexList} onChangeEvent={(input)=>setType(input)}/>
                    <CustomPickerModal select={selectMachine} onClickEvent={(e)=> setSelectMachine(e)} text={'세부 항목을 검색해주세요.'} type={indexType[type]}/>
                    {
                    ...
                    <div style={{marginTop: 12, marginLeft:180, width: 310, height: 173, lineHeight: 10, border: '1px solid #707070'}}>
                        {barcodeImg === '' ?
                            ...
                            :
                            <img src={`http://112.168.150.239:8299/api/v1/barcode/previewImg?barcode_img_name=${barcodeImg}`} style={{ width:'100%',height: '100%', float:'right'}}/>
                        }
                    </div>
                    <ListHeader title="선택 항목"/>
                    <InputContainer title={"바코드 설명"} width={180}>
                        <textarea maxLength={120} ref={textBoxRef} onChange={(e)=>setReason(e.target.value)} value={reason} style={{border:0, fontSize:14, padding:12, height:'70px', width:'calc(100% - 124px)'}} placeholder="내용을 입력해주세요 (80자 미만)"/>
                    </InputContainer>
                </div>
