          ...
          <SideInfoBox>
              ...
              <SummuryBox>
                <p>에러기록</p>
                {
                  ...
                  <div>
                    {
                      selectedMachine.history.map((m, i)=>{
                        return <p style={{color:'red', paddingBottom: 6, marginTop: 7, borderBottom: '1px solid #ffffff50'}}>{m}</p>
                      })
                    }
                  </div>
                }
                ...
                <TimeBar>
                  {
                    dummy_history.map((m, i)=>{
                      if(m.type === 'ready'){
                        ...
                        return(
                          <Packet style={{backgroundColor:'#fd6b00', width: getWidthPercent(dummy_history[i].start, dummy_history[i].end) +'%'}}>
                             <PacketTag> {'시작 : ' + moment(dummy_history[i].start).format('HH:mm')} <br/>{'종료 : ' + moment(dummy_history[i].end).format('HH:mm')} </PacketTag>
                          </Packet>
                        )
                      }
...
`
export default QdcTimeContainer;
