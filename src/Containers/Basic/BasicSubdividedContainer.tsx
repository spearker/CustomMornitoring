...
// 리스트 부분 컨테이너
const BasicSubdividedContainer = () => {
    ...
    const titleEvent = [
        {
            Name: '등록하기',
            Width: 90,
            Link: () => history.push('/basic/subdivided/register'),
        },
        // {
        //     Name: '삭제',
        //     Link: () => null
    ...
    const getList = useCallback(async (isSearch?: boolean) => {
        ...
        if (resultList) {
            const getBasic = resultList.info_list.map((v, i) => {
                ...
                const Type = transferCodeToName('subdivided', v['subdivided' + '_type'])
                return {...v, ['subdivided' + '_type']: Type}
            })
    ...
    const onClickDelete = useCallback(async (id) => {
        ...
        const tempUrl = `${API_URLS['subdivided'].delete}`
    ...
    return (
        <>
            <div style={{position: 'relative'}}>
                <OptimizedHeaderBox title={'공장 세분화'}
                                    searchBarChange={(e) => {
            ...
            <OptimizedTable widthList={LIST_INDEX['subdivided'].width}
                            indexList={LIST_INDEX['subdivided'].index}
                            valueList={list}
                            EventList={eventList}
                            mainOnClickEvent={(v) => history.push(`/basic/subdivided/register?pk=${v.pk}`)}
...
export default BasicSubdividedContainer
