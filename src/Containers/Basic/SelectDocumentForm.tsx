...
const SelectDocumentForm = ({ category, onChangeEvent }: Props) => {
  ...
  const getDocumentData = useCallback(async () => {
    ...
    const res = await getRequest('http://112.168.150.239:8299/api/v1/document/form/list?category=' + category, getToken(TOKEN_NAME))
  ...
  return (
      ...
      <form style={{ minHeight: 400 }}>
        ...
        <br/>
        <TextNotice>조회가능한 표준문서가 없다면, <span onClick={() => history.push('/basic/document/register')}>표준문서 등록</span>에서
          등록해주세요. </TextNotice>
      </form>
