...
const DocumentFormatInputList = ({ pk, loadDataUrl, onChangeEssential, onChangeOptional }: Props) => {
  ...
  const getDocumentData = useCallback(async () => {
    ...
    const res = await getRequest('http://255.255.255.255:8299/api/v1/document/form/load?pk=' + pk, getToken(TOKEN_NAME))
      ...
      <>
        {essential.length !== 0 && <ListHeader title="문서 필수 항목"/>}
        {
          essential.map((v, i) => {
            if (v.type == 0) {
              return (
                  <NormalNumberInput title={v.title} value={v.data} description={''} onChangeEvent={(input) => {
