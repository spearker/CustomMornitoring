...
const BasicMachineContainer = () => {
    ...
    const titleEvent = [
        {
            Name: '등록하기',
            Width: 90,
            Link: () => history.push('/basic/machine/register'),
        },
        // {
        //     Name: '삭제',
        //     Link: () => null
    ...
    const getList = useCallback(async (isSearch?: boolean) => {
        ...
        if (resultList) {
            const getBasic = resultList.info_list.map((v, i) => {
                ...
                const Type = transferCodeToName('machine', v['machine' + '_type'])
                return {...v, ['machine' + '_type']: Type}
            })
    ...
    const onClickDelete = useCallback(async (id) => {
        ...
        const tempUrl = `${API_URLS['machine'].delete}`
    ...
    return (
        <>
            <div style={{position: 'relative'}}>
                <OptimizedHeaderBox title={'기계 기본정보'}
                                    searchBarChange={(e) => {
            ...
            <OptimizedTable widthList={LIST_INDEX['machine'].width}
                            indexList={LIST_INDEX['machine'].index}
                            valueList={list}
                            EventList={eventList}
                            mainOnClickEvent={(v) => history.push(`/basic/machine/register?pk=${v.pk}`)}
...
export default BasicMachineContainer
