...
// 리스트 부분 컨테이너
const BasicRawMaterialContainer = () => {
  ...
  const titleEvent = [
    {
      Name: '등록하기',
      Width: 90,
      Link: () => history.push('/basic/material/register/v2'),
    },
    // {
    //     Name: '삭제',
    //     Link: () => null
  ...
  const getList = useCallback(async (isSearch?: boolean) => {
    ...
    const tempUrl = `${API_URLS['material'].filterSearch}?page=${isSearch ? 1 : page.current}&keyword=${keyword}&filter=0&limit=15`
  ...
  return (
    <>
      <div style={{position: 'relative'}}>
        <OptimizedHeaderBox title={'원자재 기본정보'}
                            searchBarChange={(e) => {
      ...
      <OptimizedTable widthList={LIST_INDEX['rawMaterial'].width}
                      indexList={LIST_INDEX['rawMaterial'].index}
                      valueList={list}
                      EventList={eventList}
                      mainOnClickEvent={(v) => history.push(`/basic/material/register/v2/${v.pk}`)}
...
export const LIST_INDEX = {
  ...
  rawMaterial: {
    title: '품목 기본정보',
    width: ['180px', '120px', '120px', '392px', '120px'],
    index: {
      texture: '재질',
...
export default BasicRawMaterialContainer
