...
const FinishMaterialContainer = () => {
  ...
  const getList = useCallback(async (isSearch?: boolean, searchOption?: number) => { // useCallback
    //TODO: 성공시
    Notiflix.Loading.Circle()
    const tempUrl = `${API_URLS['stock'].list}?type=30&filter=${filter}&page=${isSearch ? 1 : page.current}&limit=5&keyword=${searchOption !== undefined ? '' : keyword}&st=${searchOption ? searchOption : option}`
    const res = await getStockList(tempUrl)
    if (res) {
      const getStock = res.info_list.map((v, i) => {
