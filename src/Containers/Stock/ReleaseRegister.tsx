...
const ReleaseRegisterContainer = ({ match }: Props) => {
    ...
    const getData = useCallback(async()=>{
        ...
        const res = await getRequest('http://192.168.0.47:8299/api/v1/customer/view?pk=' + getParameter('pk'), getToken(TOKEN_NAME))
    ...
    const onsubmitFormUpdate = useCallback(async(e)=>{
        ...
        const res = await postRequest('http://192.168.0.47:8299/api/v1/customer/update/', data, getToken(TOKEN_NAME))
    ...
    const onsubmitForm = useCallback(async()=>{
        if(match.params.parts){
            ...
            const res = await postRequest('http://192.168.0.47:8299/api/v1/stock/parts/release/register', data, getToken(TOKEN_NAME))
            ...
            const res = await postRequest('http://192.168.0.47:8299/api/v1/stock/release/register', data, getToken(TOKEN_NAME))
        ...
        <div>
            ...
            <WhiteBoxContainer>
                    ...
                    <div style={{borderBottom: 'solid 0.5px #d3d3d3' , display:'flex', paddingTop:17, paddingBottom:17, verticalAlign: 'top'}}>
                        ...
                        <RegisterDropdown type={'string'} onClickEvent={(e: string) => setSelectType(e)} select={selectType} contents={match.params.parts ? stockList : typeList} text={'출고 구분을 선택해 주세요'} buttonWid={30} />
                    </div>
                        ...
                        <div style={{ display: 'flex', flex: 1, flexDirection: 'row', backgroundColor: '#f4f6fa', border: '0.5px solid #b3b3b3', height: 32}}>
                            <div style={{width: 'calc(100% - 100px)', display: 'table-cell'}}>
                                <div style={{marginTop: 5}}>
                                    {
                                        selectDate === ''
                                            ?<InputText>&nbsp; 출고 날짜를 선택해 주세요.</InputText>
