    ...
    <ListWrapDiv style={{
      ...
      <div style={{display: 'flex', width: '100%'}}>
        <p style={{
          ...
          textAlign: 'center',
          color: 'white',
          backgroundColor: changeStatusToColor(status)
        }}>{changeStatusToString(status !== undefined ? status : 'normal')}</p>
        {
          contents.map((v, i) => {
            return (
              <p style={{padding: '5px 10px 5px 6px', width: widths[i], color: isIconDimmed ? '#b3b3b3' : 'black'}}
                 className="p-limit"> {i !== 0 ? <span>|&nbsp;&nbsp;</span> : <span>&nbsp;&nbsp;</span>}{v}</p>
            )
          })
        }
        ...
        </div>
        {
          type == undefined || type !== 'remove' ?
            <div onClick={onClickEvent} style={{marginLeft: 'auto'}}>
              <IconSquareButton color="#e7e9eb" width="30px" imageSize="22px"
                                image={isIconDimmed ? icCheckDim : icCheck} dim={isSelected ? false : true}/>
            </div>
            :
