...
const CustomIndexInput = ({index, value, onChangeEvent, onRemoveEvent}: IProps) => {
  ...
  return ( 
        ...
        <div style={{marginTop:0, marginBottom:17, display:'flex', alignItems:'center'}}>
            <InputBox style={{width: '20%', marginRight: 8}} type="text" value={value.title} onChange={ (e: React.ChangeEvent<HTMLInputElement>): void =>{onChangeEvent({title: e.target.value, value: value.value})}} placeholder={'(항목 이름)' }/>
