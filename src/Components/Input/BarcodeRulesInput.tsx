...
const BarcodeRulesInput = ({ title, value, onChangeEvent, onRemoveEvent}: IProps) => {
  ...
  return (
        ...
        <div style={{marginTop:17, marginBottom:17, display:'flex', alignItems:'center'}}>
            ...
            <InputBox style={{width: 'calc(100% - 40px)'}} type="text" value={value} onChange={ (e)=>onChangeEvent(BarcodeType.test(e.target.value) ? e.target.value : value) } placeholder={'내용을 입력하세요.'}/>
