...
interface IProps {
  ...
  type: 'rawlot' | 'finishlot' | 'product' | 'chit' | 'contract' | 'machine' | 'project' //검색 타입
...
const NewItemPickerModal = ({select, onClickEvent, text, buttonWid, disabled, width, type, title, tableHeaderValue, mainKey, optionItems, setOption, etc, onClickLimit, isMultiSelect}: IProps) => {
                    ...
                    itemList.map((item, index) => {
                      ...
                      return (
                        <tr style={{
                          ...
                          if (isMultiSelect) {
                            if (check.indexOf(item.pk) === -1) {
                              setItemDatas([...itemDatas, item])
                            } else {
                              let tmp = itemDatas
                              tmp.splice(check.indexOf(item.pk), 1)
                              setItemDatas([...tmp])
                            }
                          } else {
                            setItemData(item)
...
export default NewItemPickerModal
